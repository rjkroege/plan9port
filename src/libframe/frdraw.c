...
// FIXME: need to worry about what's going on here with backgrounds and selections.
// I think that I'll have to re-write support for selections?
// Mental construct: we let the Acme replace the bacground for text.
void
...
frdrawsel0(Frame *f, Point pt, ulong p0, ulong p1, Image *back, Image *text)
{
	...
	for(nb=0; nb<f->nbox && p<p1; nb++){
		...
		if(x > f->r.max.x)
			x = f->r.max.x;
		draw(f->b, Rect(pt.x, pt.y, x, pt.y+f->mheight), back, nil, pt);
		if(b->nrune >= 0)
...
_frtick(Frame *f, Point pt, int ticked)
{
	...
	r = Rect(pt.x, pt.y, pt.x+FRTICKW*f->tickscale, pt.y+f->mheight);
