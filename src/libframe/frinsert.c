...
// FIXME: Some superfluous variables to clean up here.
static
Point
bxscan(Frame *f, Rune *sp, Rune *ep, Point *ppt, STag* sstp)
{
...
frsinsert(Frame* f, Rune* sp, Rune* ep, STag* sps, ulong p0)
{
	...
	Frbox *b;
	int n, n0, nn0, y;
	// int nn0, y, n00;  FIXME: these are unnecessary?
	ulong cn0;
	Image *col, *tcol;
	Rectangle r;
	static struct{
	...
	if(pt1.y > f->r.max.y)
		drawerror(f->display, "frsinsert pt1 too far");
	if(pt1.y==f->r.max.y && n0<f->nbox){
		f->nchars -= _frstrlen(f, n0);
		_frdelbox(f, n0, f->nbox-1);
	}
	if(n0 == f->nbox)
		f->nlines = (pt1.y-f->r.min.y)/f->mheight+(pt1.x>f->r.min.x);
	else if(pt1.y!=pt0.y){
		int q0, q1;
