...
static OSErr UrlHandler(const AppleEvent *event, AppleEvent *reply, long handlerRefcon);
...
static OSErr
UrlHandler(const AppleEvent *event, AppleEvent *reply, long handlerRefcon)
{
	AEDesc list;
	long len;
	char s[4000];
	...
	chk(AECountItems(&list, &len));
	memset(s, 0, 1000);
	chk(AEGetNthPtr(&list, 1, typeUTF8Text, NULL, NULL, (Ptr*)s, sizeof(s), NULL));
